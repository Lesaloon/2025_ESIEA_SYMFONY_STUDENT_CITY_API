...
class ProfileApiController extends AbstractController
{
	...
	public function updateProfileMe(
		...
		try {
			...
			$user = $this->getUser();
			if (!$user) {
				$this->logger->error('Utilisateur non trouvé dans la requête (me)');
				return $this->json([
					'success' => false,
					'message' => 'Utilisateur non authentifié'
				], JsonResponse::HTTP_UNAUTHORIZED);
			...
			$data = json_decode($request->getContent(), true);
			if (json_last_error() !== JSON_ERROR_NONE) {
				$this->logger->error('Données JSON invalides (me)', ['error' => json_last_error_msg()]);
				return $this->json([
					'success' => false,
					'message' => 'Données JSON invalides'
				], JsonResponse::HTTP_BAD_REQUEST);
			...
			$this->logger->info('Profil (me) mis à jour avec succès', [
				'user_id' => $user->getId(),
				'email' => $user->getEmail()
			]);
			...
			$this->logger->error('Erreur lors de la mise à jour du profil (me)', [
				'error' => $e->getMessage(),
				'trace' => $e->getTraceAsString()
			]);
			return $this->json([
