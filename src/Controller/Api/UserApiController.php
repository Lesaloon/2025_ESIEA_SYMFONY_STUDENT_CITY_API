...
class UserApiController extends AbstractController
{
	...
	public function getCurrentUser(): JsonResponse
	{
		...
		if (!$user instanceof \App\Entity\User) {
			// Try to fetch the User entity from the database using the username/email
			$userEntity = $this->entityManager->getRepository(User::class)->findOneBy(['email' => $user->getUserIdentifier()]);
			if (!$userEntity) {
				return $this->json([
					'success' => false,
					'message' => 'Utilisateur non trouvé'
	...
	#[Route('/api/users/me/update', name: 'api_user_update_me', methods: ['PUT'])]
	#[IsGranted('IS_AUTHENTICATED_FULLY')]
	public function updateCurrentUser(Request $request, UserPasswordHasherInterface $passwordHasher): JsonResponse
	{
		$user = $this->getUser();
		if (!$user instanceof \App\Entity\User) {
